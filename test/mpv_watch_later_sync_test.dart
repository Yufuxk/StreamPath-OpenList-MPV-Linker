...
void main() {
  ...
  group('duration= 解析与读取（已看完判定用）', () {
    ...
    test('readDurationSeconds 按 MD5 直查', () async {
      ...
      File('${dir.path}${Platform.pathSeparator}${MpvWatchLaterSync.md5FileName(url)}')
  ...
  group('readStartSeconds 目录扫描匹配', () {
    ...
    Future<void> writeRecord(String fileName, String content) =>
    ...
    test('MD5 文件名直查（HTTP 流无注释行，mpv 实测格式）', () async {
      ...
      const sync = MpvWatchLaterSync();
      expect(await sync.readStartSeconds(dir, url), closeTo(0.465951, 0.000001));
    });

    test('本地路径记录（带注释行）也可匹配', () async {
      await writeRecord('FCD93DCF75B5974306E37F149FC858AF',
          '# C:\\Users\\YX\\Desktop\\movie.flac\nstart=3660.5\n');

      const sync = MpvWatchLaterSync();
      expect(await sync.readStartSeconds(dir, r'C:\Users\YX\Desktop\movie.flac'),
          closeTo(3660.5, 0.001));
    });

    test('sanitize 命名（write-filename-in-watch-later-config）也可匹配', () async {
      // 文件名与 URL 无关，匹配完全依赖首行注释。
      await writeRecord('wl_sanitized_a.mp4',
          '# http://host/dav/a.mp4\nstart=90\n');

      const sync = MpvWatchLaterSync();
      expect(await sync.readStartSeconds(dir, 'http://host/dav/a.mp4'),
          closeTo(90, 0.001));
    });
    ...
    test('注释行带引号也可匹配', () async {
      ...
      const sync = MpvWatchLaterSync();
      expect(await sync.readStartSeconds(dir, 'http://host/b.mp4'),
          closeTo(12, 0.001));
    });
    ...
    test('无关文件（其他 URL/无注释）不影响匹配', () async {
      ...
      const sync = MpvWatchLaterSync();
      expect(await sync.readStartSeconds(dir, 'http://host/target.mp4'), isNull);
    });
    ...
    test('目录不存在返回 null', () async {
      ...
      expect(
        await sync.readStartSeconds(
            Directory('${dir.path}${Platform.pathSeparator}missing'), 'http://h/x.mp4'),
        isNull,
      );
    });
  });
